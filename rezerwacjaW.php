...
if(isset($_POST['liczbaOsob'])){ //sprawdź poprawność formularza
  ...
  if ($_POST['liczbaOsob'] <= 0) {
    ...
    try {
        ...
        if(isset($_POST['saleDostepne'])){
            $nazwaSali=$_POST['saleDostepne'];
            
        }
        else{
            $nazwaSali=$_POST['saleDostepne2'];
        }
        ...
        if ($polaczenie->connect_errno != 0) { //obsluga bledu
            ...
            $pobierzMaxLiczbeOsob = $polaczenie->query('SELECT liczbaOsob from sale where nazwa=' . '"' . $nazwaSali . '"');
            ...
            if($_POST['liczbaOsob']<=$maxLiczbaOsob){
                ...
                $_SESSION['e_liczba']="Maksymalna liczba osób, które pomieszczą się w tej sali wynosi " . $maxLiczbaOsob . ".";
...
<head>
    ...
    <script type="text/javascript">
        ...
        //             document.getElementById("demo").innerHTML = nazwaSali;
        ...
        //                 // document.getElementById("demo").innerHTML = nazwaSali;
        // $('#szczegoly').append('<li><a href="'+nazwaSali+'.php" class="links" target="_blank">'+ sala.nazwa +
        // ' - sprawdź szczegóły</a></li>');
...
<body>
    ...
    <section class="form-wall">
        ...
        try {
            ...
            if ($polaczenie->connect_errno != 0) { //obsluga bledu
                ...
                $pobierzMaxLiczbeOsob = $polaczenie->query('SELECT liczbaOsob from sale where nazwa=' . '"' . $_POST['wybranaSala'] . '"');
                $row = mysqli_fetch_array($pobierzMaxLiczbeOsob);
                $maxLiczbaOsob = $row['liczbaOsob'];
                    }
        ...
        ?>
            ...
            <input id="liczba-os" type="text" placeholder="maksymalna liczba osób=<?php echo isset($maxLiczbaOsob) ? $maxLiczbaOsob : '' ?>" onblur="getVal()"
            ...
            <?php
              echo '<option>' . $_POST['wybranaSala'] . '</option>';
            ?>
  ...
  for ($i = 10; $i < 60; $i += 5) {
    ...
    echo 'value="' . $i . '">' . $i . '</option>';
            ...
            <input type="hidden" name="wybranaSala" value="<?php echo isset($_POST['wybranaSala']) ? $_POST['wybranaSala'] : '' ?>">
            <input type="hidden" name="saleDostepne2" value="<?php echo isset($_POST['wybranaSala']) ? $_POST['wybranaSala'] : '' ?>">
        </form>
